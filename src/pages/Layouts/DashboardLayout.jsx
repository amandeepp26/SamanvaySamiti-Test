    ...
    <>
      ...
      <div className="lg:hidden flex justify-center my-3">
        <button
          ...
          className="hover:bg-primary-normal py-1 px-4 text-primary-normal hover:text-white rounded border-2 border-primary-normal text-xl flex items-center gap-1"
        >
      ...
      {showSidebar && (
        <div
          className="lg:hidden fixed top-0 left-0 w-full h-full bg-[#000000b0]"
          onClick={toggleSidebar}
        />
      )}
      ...
      <div className="lg:grid lg:grid-cols-4 lg:gap-3">
        <div
          className="bg-gray-50 fixed lg:relative w-80 lg:w-full top-0 left-0 z-50"
          style={sidebarStyles}
          onClick={() => (!isLargeScreen ? toggleSidebar() : "")}
        >
          <div className="h-screen relative overflow-y-auto">
            {showSidebar && (
              <span
                onClick={toggleSidebar}
                className="absolute top-0 right-0 text-red-500 text-3xl p-5 hover:bg-[#ff000012] lg:hidden cursor-pointer"
              >
                <MdClose />
              </span>
            )}
