...
const CheckoutForm = ({ newItem }) => {
  ...
  useEffect(() => {
    if (totalPrice > 0) {
      axiosSecure
        .post("/create-stripe-payment-intent", { price: totalPrice })
        .then((res) => {
  ...
  return (
    <div className="max-w-lg mx-auto">
      <form onSubmit={handleCheckout}>
        ...
        />
        <button
          className="bg-black text-white py-1 px-3 rounded-md my-8 cursor-pointer"
          type="submit"
      ...
      <p className="text-green-500 py-5">
        {transactionId && `Paid 500 TK - Transaction ID : ${transactionId}`}{" "}
      </p>
    </div>
  );
};
